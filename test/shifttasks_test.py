...
def stdout(release_build):
    return release_build.shell.execute("bitbake core-image-minimal -c listtasks").stdout
...
def test_do_checktestall(stdout, release_build):
    assert stdout.contains("do_checktestall")
    o = release_build.shell.execute("bitbake core-image-minimal -c checktestall")
    assert o.stdout.contains("WARNING: core-image-minimal-1.0-r0 do_checktestall: No recipes found to run 'do_checktest' task.")
