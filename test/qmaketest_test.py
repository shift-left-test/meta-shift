...
def stdout(report_build):
    return report_build.shell.execute("bitbake qmake-project -c verify").stdout


@pytest.fixture(scope="module")
def report(report_build):
    report_build.files.remove("report")
    assert report_build.shell.execute("bitbake qmake-project -c verify").stderr.empty()
    return report_build


def test_do_checktest(stdout, report):
    assert stdout.matches("qmake-project-1.0.0-r0 do_checktest:[ ]+Mutant Generation Summary")
    assert stdout.matches("qmake-project-1.0.0-r0 do_checktest:[ ]+Mutation Score Report")
...
def test_do_verify(test_build):
    o = test_build.shell.execute("bitbake qmake-project -c verify")
