...
def test_populate_sdk_target(bare_build):
    ...
    stdout = bare_build.shell.execute("bitbake packagegroup-core-standalone-sdk-target -e | grep ^RDEPENDS:packagegroup-core-standalone-sdk-target=").stdout
    assert stdout.containsAll("fff", "googletest")


def test_populate_sdk_host(bare_build):
    # Simply check whether required packages exist in a packagegroup, instead of generating the sdk file.
    stdout = bare_build.shell.execute("bitbake nativesdk-packagegroup-sdk-host -e | grep ^RDEPENDS:nativesdk-packagegroup-sdk-host=").stdout
