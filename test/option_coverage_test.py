...
def test_cmake_project_coverage_filter(report_build):
    with report_build.files.conf() as conf:
        ...
        assert coverage["coverage/packages/package/classes/class"]["filename"] != "minus/src/minus.cpp"
        assert coverage["coverage/packages/package/classes/class"]["filename"] != "plus/src/plus.cpp"
...
def test_qmake_project_coverage_filter(report_build):
    with report_build.files.conf() as conf:
        ...
        assert coverage["coverage/packages/package/classes/class"][0]["filename"] != "minus/src/minus.cpp"
        assert coverage["coverage/packages/package/classes/class"][0]["filename"] != "plus/src/plus.cpp"
