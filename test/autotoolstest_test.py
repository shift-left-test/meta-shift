...
def report(report_build):
    ...
    assert report_build.shell.execute("bitbake autotools-project -c report").stderr.empty()
    return report_build
...
def test_do_checkcode(stdout, report):
    ...
    with report.files.readAsHtml("report/autotools-project-1.0.0-r0/checkcode/index.html") as data:
        assert data["html/body/h1"] == "Sage Report"
    with report.files.readAsJson("report/autotools-project-1.0.0-r0/checkcode/sage_report.json") as data:
...
def test_do_test_shuffle(test_build):
    with test_build.files.conf() as conf:
        ...
        first = test_build.shell.execute("bitbake autotools-project -c test").stdout
        second = test_build.shell.execute("bitbake autotools-project -c test").stdout
        assert first != second
