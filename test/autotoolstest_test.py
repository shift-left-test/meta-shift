...
def stdout(report_build):
    return report_build.shell.execute("bitbake autotools-project -c verify").stdout


@pytest.fixture(scope="module")
def report(report_build):
    report_build.files.remove("report")
    assert report_build.shell.execute("bitbake autotools-project -c verify").stderr.empty()
    return report_build
...
def test_do_verify(test_build):
    o = test_build.shell.execute("bitbake autotools-project -c verify")
