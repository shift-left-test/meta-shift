...
| `SHIFT_REPORT_DIR` | Directory for generated reports; each recipe writes to `<dir>/<PF>/{test,coverage,checktest}/`, and each task directory includes a `metadata.json` recording the recipe source path (`S`). Unset = no report files are written, but every task still runs and streams results to the console — mutation testing then runs in full, so bound it with `SHIFT_CHECKTEST_LIMIT` | |
