...
1.  **Clone the repository:**
    ```bash
    git clone -b whinlatter https://github.com/shift-left-test/meta-shift.git
    ```
...
| `SHIFT_REPORT_DIR` | Directory for generated reports; each recipe writes to `<dir>/<PF>/{test,coverage,checktest}/` (CMake recipes write a single `test/report.xml` produced by `ctest --output-junit`; Autotools/QMake write the test runner's own XML files), and each task directory includes a `metadata.json` recording the recipe source path (`S`). Unset = no report files are written, but every task still runs and streams results to the console — mutation testing then runs in full, so bound it with `SHIFT_CHECKTEST_LIMIT` | |
...
| `SHIFT_TEST_PARALLEL_JOBS` | Number of tests to run in parallel (CMake/ctest only; passed to `ctest --parallel`). Empty = serial; set a number or `${@oe.utils.cpu_count()}` for all host cores. Test results come from `ctest --output-junit`, so parallel runs keep a single consistent report | |
...
this repository (based on **Ubuntu 24.04**):
```bash
