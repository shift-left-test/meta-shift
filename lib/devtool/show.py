...
def parse_metadata(tinfoil, recipe):
    if recipe:
        ...
        appendfiles = tinfoil.cooker.collection.get_file_appends(recipe)
        parser = bb.cache.NoCache(tinfoil.cooker.databuilder)
        return parser.loadDataFull(recipefile, appendfiles)
    else:
